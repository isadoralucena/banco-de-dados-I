...
CREATE OR REPLACE TRIGGER trigger_impedir_criacao_novo_perfil_quando_numMaxTelas_atingido
 BEFORE INSERT ON Perfil 
 FOR EACH ROW
 DECLARE 
    num_max_telas NUMBER;
    num_telas_atual NUMBER;
 BEGIN
    SELECT assi.max_telas INTO num_max_telas FROM Assinatura assi JOIN Usuario usr ON usr.id_assinatura = assi.id WHERE usr.id = :NEW.id_usuario;
    SELECT COUNT(*) INTO num_telas_atual FROM Perfil WHERE id_usuario = :NEW.id_usuario;

    IF num_telas_atual >= num_max_telas THEN 
        RAISE_APPLICATION_ERROR(-20001, 'Você já atingiu o número máximo de perfis para essa assinatura. Não é possível adicionar mais perfis!'); 
    END IF;
END;
/
